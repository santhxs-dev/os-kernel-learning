...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    res = pathparser_get_drive_by_path(&tmp_path); // 0
    if (res < 0)
    {
        goto out;
    }
    ...
    struct path_part* first_part = pathparser_parse_path_part(NULL, &tmp_path); // shell.exe?
    if (!first_part)
    {
        goto out;
    }
